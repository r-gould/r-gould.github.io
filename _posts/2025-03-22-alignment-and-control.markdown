...
* Summarization of the AI's output by an assistant AI in a way thats curated to the supervisor's ability.
* Debate allows multiple instances of the same AI system to argue for different sides of a question (e.g. "does this response follow the user's instruction?"), producing a debate transcript that the supervisor can base their judgement upon.
...
* Even with perfect supervision ability and perfectly specified reward models, we still must face the problem of goal misgeneralization: an AI system may converge to a strategy that performs well in training yet the strategy fails at deployment due to misgeneralization (e.g. the strategy optimizes for a goal that spuriously correlates with our intended goal).
* If the training setting is not sufficiently "diverse" (or sufficiently adversarial) or accurate to deployment, the system will learn such shortcuts that fail to generalize.
...
* Another example of goal misgeneralization in LMs is [emergent misalignment], which finds that finetuning on insecure code results in generically bad outputs: there are insufficient pressures in this dataset to prevent shortcuts of "always output bad things".
* But some RL systems can be suprisingly robust out-of-distribution: self-play chess agents like MuZero can beat humans, even though they had never seen a human play during training. Here there is a mismatch between training and deployment, however the system still learns the correct general strategy due to the adversarial nature of self-play RL.
