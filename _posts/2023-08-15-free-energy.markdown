...
Typically, however, $$p(o) = \int_{\mathcal{X}} p(x, o) dx$$ is intractable, so we consider a **variational posterior** $$q(x\vert o)$$ that we want to effectively approximate $$p(x\vert o)$$. The natural objective to achieve this 
...
* See [Millidge et al. (2020)](https://arxiv.org/abs/2004.08128) for a justification of FEEF as a natural extension of the free energy to action.
...
* The second term, $$\mathbb{E}_{q(o|\pi)}[D_{KL}(q(x|o, \pi) || q(x|\pi))]$$, describing the **expected information gain under observation**, will be incentivized to increase.
...
[Tschantz et al. (2020)](https://arxiv.org/abs/2002.12636) arrive at a different approximation for $$\mathcal{F}_{\pi}$$. I now compare my approach detailed above, and theirs, by implementing my approach into the codebase for their paper, with the same hyperparameters across the two approaches. 
...
$$\frac{\partial F}{\partial \hat{\mu}_i} = \Sigma_i^{-1} \epsilon_i - 1\{i \geq 1\} \frac{\partial \mu_{i-1}}{\partial \hat{\mu}_i} \Sigma_{i-1}^{-1} \epsilon_{i-1}$$
